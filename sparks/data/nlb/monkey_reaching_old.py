...
def process_dataset(dataset_path: os.path):
    ...
    return align_data, unique_ids, trial_ids
...
def make_monkey_reaching_dataset(dataset_path: os.path,
    ...
    align_data, y_trial_data, unique_ids, trial_ids = process_dataset(dataset_path)
    ...
    def __init__(
        ...
        if y_keys == 'direction':
            ...
            if subkeys is not None:
                y_trial_data = np.vstack([np.vstack([align_data[i][y_keys][subkeys].to_numpy()[trial_ids[i] == idx][None, :]
                                                     for idx in self.indices[i]]) for i in range(len(align_data))]).transpose([0, 2, 1])
            else:
                y_trial_data = np.vstack([np.vstack([align_data[i][y_keys].to_numpy()[trial_ids[i] == idx][None, :]
                                                     for idx in self.indices[i]]) for i in range(len(align_data))]).transpose([0, 2, 1])
